...
function init(env, ctx) {
    ...
    dataloader.update = function update(ddata, opts, done) {
        ...
        async.parallel([
            ...
            , loadAgeTreatments.bind(null, ddata, ctx)
...
function loadAgeTreatments (ddata, ctx, callback) {
  var dateRange = {
    $gte: new Date(ddata.lastUpdated - (ONE_DAY * 60)).toISOString()
  };
  ...
  var tq = {
    find: {
      eventType: {
        $in: [ 'Sensor Start', 'Sensor Change','Insulin Change', 'Site Change', 'Follicular Phase']
      }
      , created_at: dateRange
    }
    , sort: {created_at: -1}
...
function loadAgeTreatments(ddata, ctx, callback) {
    async.parallel([
        ...
        ,loadLatestSingle.bind(null, ddata, ctx, 'Follicular Phase')
    ], callback);
}
...
function loadLatestSingle(ddata, ctx, dataType, callback) {
    ...
    var dateRange = {
        $gte: new Date(ddata.lastUpdated - (constants.ONE_DAY * 60)).toISOString()
    };
