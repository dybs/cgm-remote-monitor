...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
  ...
  table.append($('<tr>').css('background','gray')
    .append($('<th>'))
    .append($('<th>').css('width','80px').attr('align','left').append(translate('Date')))
    ...
    .append($('<th>').css('width','300px').attr('align','left').append(translate('Food Type')))
  );
  
  sorteddaystoshow.forEach(function (day) {
    table.append($('<tr>')
      .append($('<td>').attr('colspan','14').css('background','lightgray')
    ...
    for (var t=0; t<treatments.length; t++) {
      ...
      table.append($('<tr>').addClass('border_bottom')
        ...
        .append($('<td>').append(tr.foodType ? tr.foodType : ''))
      );
    }
  });
